...
client.connect((err) => {
	const serviceCollection = client
		...
		.collection(`${process.env.DB_COLLECTION_1}`);
	const feedbackCollection = client
		.db(`${process.env.DB_NAME}`)
		.collection(`${process.env.DB_COLLECTION_2}`);
	const messageCollection = client
		.db(`${process.env.DB_NAME}`)
		.collection(`${process.env.DB_COLLECTION_3}`);
	const customerOrdersCollection = client
		.db(`${process.env.DB_NAME}`)
		.collection(`${process.env.DB_COLLECTION_4}`);
	const adminCollection = client
		.db(`${process.env.DB_NAME}`)
		.collection(`${process.env.DB_COLLECTION_5}`);
	...
	app.post('/addService', (req, res) => {
		...
		serviceCollection
			.insertOne({ taskName, description, src, image })
			.then((result) => {
	...
	app.post('/privateMessage', (req, res) => {
		message = req.body;
		messageCollection.insertOne(message).then((result) => {
			res.send(result.insertedCount > 0);
		});
	});
	...
	app.post('/placeOrder', (req, res) => {
		order = req.body;
		customerOrdersCollection.insertOne(order).then((result) => {
			res.send(result.insertedCount > 0);
		});
	});
	...
	app.get('/customerOrders', (req, res) => {
		...
		adminCollection.find({ email: email }).toArray((err, admin) => {
			...
			customerOrdersCollection.find(filter).toArray((err, documents) => {
				res.send(documents);
			});
		});
	});
